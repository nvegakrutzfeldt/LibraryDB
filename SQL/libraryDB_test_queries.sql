...
UPDATE `books`
    SET `book_amount_owned` = `book_amount_owned` + 1
...
UPDATE `books`
    SET `book_amount_owned` = `book_amount_owned` - 1
WHERE `book_isbn` = '8432298420'
AND `book_amount_owned` > 0;

# Register an account (add users)
...
VALUES
    ('Joey Wheeler', '18000487774', 'jowheel@gmail.com', '12345', 'user',
...
INSERT INTO `users`(`user_name`, `user_phone_number`, `user_email`, `user_password`, `user_type`, `address_id`)
VALUES
    ('Peter Griffin', '18009382311', 'beter@gmail.com', 'lois', 'user',
    (SELECT `address_id` FROM `address`
    WHERE `address_number` = '1' AND
    `address_street` = '1st street' AND
    `address_city` = 'Little Rock' AND
